		...
		var cookieApp = {
			...
			body: function () {
				$("body").append(
					...
					'<button class="disagreement-btn">' + settings.buttonText2 + '</button>' +
					'</div>' +
					'</div>' +
					'</div >' +
					'</div >'
			...
			painter: function () {
				var cb_message = $('.cookie-bubble .cb-message span'),
					cb_button = $('.cookie-bubble .agreement-btn,.cookie-bubble .agreement-btn'),
			...
			checkCookie: function () {
				...
				if (get_cookie != "" && get_cookie != null) {
					...
					if (cookieApp.getQuery('test')) {
						cookieApp.setCookie(private_opt.cookieName, '', -1)
						location.replace(location.href.split('?')[0])
					}
				} else {
					console.log('consent ask')
					cookieApp.body();
					cookieApp.painter();
					setTimeout(function () {
					...
					$('.cookie-bubble .agreement-btn,.cookie-bubble .disagreement-btn').click(function (event) {
						if ($(this).hasClass('agreement-btn'))
